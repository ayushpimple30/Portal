...
class QuizAttempt(db.Model):
    __tablename__ = 'quiz_attempts'; id = db.Column(db.Integer, primary_key=True); user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True); module_id = db.Column(db.Integer, db.ForeignKey('modules.id', ondelete='CASCADE'), nullable=False, index=True); score = db.Column(db.Integer, nullable=False); total_questions = db.Column(db.Integer, nullable=False); percentage = db.Column(db.Float, nullable=False); passed = db.Column(db.Boolean, nullable=False); started_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False); submitted_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False); duration_seconds = db.Column(db.Integer, default=0, nullable=False)
